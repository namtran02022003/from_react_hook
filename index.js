    ...
    if (dataLocal.length > 0) {
        ...
        let td = dataLocal.map(datas => {
            ...
            return `<tr>${htmltd.join('')} <td>
                <button type="button" class="btn-edit" onclick="editUser(${datas.id})">edit</button></td>
                 <td><button class="btn-remove" type="button" onclick="removeUser(${datas.id})">remove</button></td<tr/>`
        })
    ...
    for (let i = 0; i < length; i++) {
        ...
        if (element.type == 'text') {
            if (element.name == "phoneNumber") {
                if (/^(1\s|1|)?((\(\d{3}\))|\d{3})(\-|\s)?(\d{3})(\-|\s)?(\d{4})$/.test(element.value)) {
                    datas[element.getAttribute('id')] = element.value
                } else {
                    element.focus()
                    element.parentElement.querySelector('.err').innerHTML = "Phone number không hợp lệ"
                    return
                }
            } else {
                if (element.value.trim().length > 20) {
                    element.focus()
                    element.parentElement.querySelector('.err').innerHTML = "Vui lòng chỉ nhập tối đa 20 ký tự"
                    return
                } else if (element.value.trim().length < 6) {
                    element.focus()
                    element.parentElement.querySelector('.err').innerHTML = "Vui lòng nhập tối thiểu 6 ký tự!"
                    return
                }
                datas[element.getAttribute('id')] = element.value
            }
        ...
        if (element.type == 'password') {
            if (element.value.trim().length > 20) {
                element.focus()
                element.parentElement.querySelector('.err').innerHTML = "Vui lòng nhập tối đa 20 ký tự"
                return
            }
        ...
        if (element.type == 'date') {
            if (element.value) {
                ...
                element.focus()
                element.parentElement.querySelector('.err').innerHTML = "Vui lòng nhập trường này!"
                return
            }
        }
        if (element.type == 'email') {
            if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(element.value)) {
                ...
                element.focus()
                element.parentElement.querySelector('.err').innerHTML = "Email không hợp lệ!"
                return
            }
        }
